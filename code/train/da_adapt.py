...
def comb_adversarial_training_stage(args, target_train_features, target_train_examples, source_train_features,
    ...
    def sample_source_batch_top3(source_data, source_q_type_dict, sample_pointer, q_type_probs):
        ...
        input_ids_source, input_masks_source, segment_ids_source, start_positions_source, end_positions_source, \
            q_types_source, q_type_probs_source = [], [], [], [], [], [], []  # q_type_probs_source 추가
        for idx in output_idx:
            ...
            q_type_probs_source.append(source_data[idx][6].unsqueeze(0))  # 추가
        ...
        return torch.vstack(input_ids_source), torch.vstack(input_masks_source), torch.vstack(segment_ids_source), \
            ...
            torch.vstack(q_type_probs_source)  # q_type_probs는 2D이므로 vstack 사용
    ...
    source_q_types = []
    source_q_type_probs = []  # 추가: 소스 데이터의 q_type_prob
    source_q_type_dict = {
        0: [],
        1: [],
        2: [],
    ...
    for idx, f in enumerate(source_train_features):
        ...
        # q_type_prob가 있으면 사용, 없으면 one-hot 생성
        if hasattr(f, 'q_type_prob') and f.q_type_prob is not None:
            source_q_type_probs.append(f.q_type_prob)
        else:
            prob = [0.0] * 6
            prob[f.q_type] = 1.0
            source_q_type_probs.append(prob)
    source_q_types = torch.tensor(source_q_types, dtype=torch.long)
    source_q_type_probs = torch.tensor(source_q_type_probs, dtype=torch.float)
    ...
    source_data = TensorDataset(source_input_ids, source_input_mask, source_segment_ids, source_start_positions,
        source_end_positions, source_q_types, source_q_type_probs)
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        batch = tuple(t.to(device) for t in batch)  # q_type_probs 포함
        ...
        input_ids, input_masks, segment_ids, start_positions, end_positions, q_types, q_type_probs = batch
        ...
        input_ids_source, input_masks_source, segment_ids_source, start_positions_source, end_positions_source, q_types_source, q_type_probs_source = batch_source
        # 타겟 + 소스 concat
        ...
        q_type_probs_concat = torch.cat((q_type_probs, q_type_probs_source), 0)
        # QC4QA loss 계산 및 역전파
        # loss = model.forward_ours(input_ids, segment_ids, input_masks, start_positions,
        #         end_positions, q_types, lambda_c=args.lambda_c)
        loss = model.forward_ours(input_ids, segment_ids, input_masks, start_positions,
                end_positions, q_types, q_type_probs=q_type_probs_concat, lambda_c=args.lambda_c)
...
def prepare_model(args, device):
    ...
    model = BertForQuestionAnsweringWeightQCQA.from_pretrained(args.bert_model, state_dict=model_state_dict, args=args)
    model.to(device)
    return model
