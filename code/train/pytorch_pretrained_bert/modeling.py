...
PRETRAINED_MODEL_ARCHIVE_MAP = {
    ...
    'bert-large-cased': "https://s3.amazonaws.com/models.huggingface.co/bert/bert-base-cased.tar.gz",
...
# Notice FusedLayerNorm requires half precision and is problematic in O1 training
# try:
...
class BertEmbeddings(nn.Module):
    ...
    def forward_ours(self, input_ids, token_type_ids=None, synonym_ids=None, synonym_coeffs=None):
        seq_length = input_ids.size(1)
        position_ids = torch.arange(seq_length, dtype=torch.long, device=input_ids.device)
        position_ids = position_ids.unsqueeze(0).expand_as(input_ids)
        if token_type_ids is None:
        ...
        if synonym_ids is not None and synonym_coeffs is not None:
            for i in range(synonym_ids.size(-1)):
                if i == 0:
                    words_embeddings = synonym_coeffs[:, :, i].unsqueeze(-1) * self.word_embeddings(synonym_ids[:, :, i])
                else:
                    words_embeddings += synonym_coeffs[:, :, i].unsqueeze(-1) * self.word_embeddings(synonym_ids[:, :, i])
        else:
            words_embeddings = self.word_embeddings(input_ids)
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask, return_attn=False):
        if return_attn:
            self_output, attention_probs = self.self(input_tensor, attention_mask, True)
        else:
...
class BertOutput(nn.Module):
    ...
    def forward_ours(self, hidden_states, input_tensor, attn_weights, cutoff_mask, cutoff_ratio):
        if cutoff_mask is None:
            return self.forward(hidden_states, input_tensor)
        
        hidden_states = self.dense(hidden_states)
        attn_weights = attn_weights.masked_fill(cutoff_mask==0, -1e9)
        attn_probs = nn.functional.softmax(attn_weights, dim=-1)

        splits = (cutoff_mask == 1) * torch.arange(cutoff_mask.shape[1], 0, -1).to(cutoff_mask.device)
        context_starts = torch.sort(splits, -1, descending=True)[1][:, 0]
        splits = (cutoff_mask == 1) * torch.arange(0, cutoff_mask.shape[1]).to(cutoff_mask.device)
        context_ends = torch.sort(splits, -1, descending=True)[1][:, 0] + 1  # no ids after context, dropout for consistency

        cutoff_starts = torch.distributions.Categorical(attn_probs).sample()
        cutoff_lens = (torch.sum(cutoff_mask, dim=-1) * cutoff_ratio).int()
        cutoff_lens = cutoff_lens.masked_fill(cutoff_lens <= 1, 2)
        cutoff_starts = cutoff_starts - (cutoff_lens / 2).int()
        cutoff_starts = torch.max(context_starts, cutoff_starts)
        cutoff_ends = cutoff_starts + cutoff_lens.int()
        cutoff_ends = torch.min(context_ends, cutoff_ends)
        for idx in range(hidden_states.size(0)):
            hidden_states[idx, :context_starts[idx]] = self.dropout(hidden_states[idx, :context_starts[idx]])
            hidden_states[idx, context_ends[idx]:] = self.dropout(hidden_states[idx, context_ends[idx]:])
            hidden_states[idx, cutoff_starts[idx]:torch.min(cutoff_ends[idx], context_ends[idx])] = 0.
        hidden_states = self.LayerNorm(hidden_states + input_tensor)
        return hidden_states
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, return_attn=False):
        if return_attn:
            attention_output, attention_probs = self.attention(hidden_states, attention_mask, True)
            avg_attn_weights = torch.mean(torch.sum(attention_probs, dim=-2), dim=1).detach()
        else:
            attention_output = self.attention(hidden_states, attention_mask)
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output(intermediate_output, attention_output)
        if return_attn:
            return layer_output, avg_attn_weights
        else:
            return layer_output

    def forward_ours(self, hidden_states, attention_mask, cutoff_mask, cutoff_ratio):
        attention_output, attention_probs = self.attention(hidden_states, attention_mask, return_attn=True)
        avg_attn_weights = torch.mean(torch.sum(attention_probs, dim=-2), dim=1).detach()
        intermediate_output = self.intermediate(attention_output)
        layer_output = self.output.forward_ours(intermediate_output, attention_output, avg_attn_weights, cutoff_mask, cutoff_ratio)
        return layer_output
...
class BertEncoder(nn.Module):
    ...
    def forward_ours(self, hidden_states, attention_mask, cutoff_mask=None, cutoff_ratio=0., output_all_encoded_layers=True):
        all_encoder_layers = []
        for idx, layer_module in enumerate(self.layer):
            if idx == len(self.layer) - 1:
                hidden_states, avg_attn_weights = layer_module(hidden_states, attention_mask, return_attn=True)
            else:
                hidden_states = layer_module.forward_ours(hidden_states, attention_mask, cutoff_mask, cutoff_ratio)
            if output_all_encoded_layers:
                all_encoder_layers.append(hidden_states)
        if not output_all_encoded_layers:
            all_encoder_layers.append(hidden_states)
        return all_encoder_layers, avg_attn_weights
...
class PreTrainedBertModel(nn.Module):
    ...
    @classmethod
    def from_pretrained(cls, pretrained_model_name, state_dict=None, cache_dir=None, args=None, *inputs, **kwargs):
        """
...
class BertModel(PreTrainedBertModel):
    ...
    def __init__(self, config):
        ...
        # self.layer_norm = torch.nn.LayerNorm(config.max_seq_length)
    ...
    def forward_ours(self, input_ids, token_type_ids=None, attention_mask=None, synonym_ids=None,
                     synonym_coeffs=None, cutoff_mask=None, cutoff_ratio=0., output_all_encoded_layers=True):
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        ...
        embedding_output = self.embeddings.forward_ours(input_ids, token_type_ids, synonym_ids, synonym_coeffs)
        # embedding_output = self.layer_norm(embedding_output)
        encoded_layers, avg_attn_weights = self.encoder.forward_ours(embedding_output, extended_attention_mask, cutoff_mask, 
                                                                     cutoff_ratio, output_all_encoded_layers=output_all_encoded_layers)
        sequence_output = encoded_layers[-1]
        # pooled_output = self.pooler(sequence_output)
        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]
        return encoded_layers, avg_attn_weights
...
class BertForQuestionAnsweringQC4QA(PreTrainedBertModel):
    ...
    """
    def __init__(self, config):
        super(BertForQuestionAnsweringQC4QA, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if config.use_BN:
    ...
    def forward_ours(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, 
                     end_positions=None, q_types=None, lambda_c=0.):
        
        # _, splits = (input_ids == 102).max(-1)
        # cutoff_mask = (token_type_ids == 1) * attention_mask
        # cutoff_mask[torch.arange(cutoff_mask.size(0)), splits] = 0
        # for i in range(cutoff_mask.size(0)):
        #     cutoff_mask[i][cutoff_mask[i].nonzero().squeeze()[-1]] = 0
        ...
        if start_positions is not None and end_positions is not None:
            ...
            # notice it doesn't matter if source batch comes first or target batch comes first
...
class BertForQuestionAnsweringQADA(PreTrainedBertModel):
    ...
    """
    def __init__(self, config):
        super(BertForQuestionAnsweringQADA, self).__init__(config)
        self.bert = BertModel(config)
        # TODO check with Google if it's normal there is no dropout on the token classifier of SQuAD in the TF version
        # self.dropout = nn.Dropout(config.hidden_dropout_prob)
        if config.use_BN:
    ...
    def forward_ours(self, input_ids, token_type_ids=None, attention_mask=None, start_positions=None, 
                     end_positions=None, synonym_ids=None, synonym_coeffs=None, cutoff_ratio=0., lambda_c=0.):
        ...
        for i in range(cutoff_mask.size(0)):
            cutoff_mask[i][cutoff_mask[i].nonzero().squeeze()[-1]] = 0
        sequence_output, avg_attn_weights = self.bert.forward_ours(input_ids, token_type_ids, attention_mask, synonym_ids, synonym_coeffs, 
                                                                   cutoff_mask, cutoff_ratio, output_all_encoded_layers=False)
        if self.use_BN:
            sequence_output = self.norm_layer(sequence_output)
        logits = self.qa_outputs(sequence_output)
        start_logits, end_logits = logits.split(1, dim=-1)
        ...
        if start_positions is not None and end_positions is not None:
            ...
            end_loss = loss_fct(end_logits[:end_positions.size(0)], end_positions)
            if valid_idx.nelement() > 0:
                total_loss = (start_loss + end_loss) / 2 + lambda_c * caqa_loss
            else: 
                total_loss = (start_loss + end_loss) / 2
            return total_loss 
        else:
