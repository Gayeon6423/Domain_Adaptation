...
class PyTorchClassifier(object):
    ...
    def predict(self, devX):
        ...
        # softmax를 적용하여 확률값 반환
    def predict_proba(self, devX):
        self.model.eval()
        if not isinstance(devX, torch.cuda.FloatTensor):
            devX = torch.FloatTensor(devX).cuda()
        yhat = np.array([])  # 예측 결과를 저장할 배열
        with torch.no_grad():
            for i in range(0, len(devX), self.batch_size):
                Xbatch = devX[i:i + self.batch_size]
                output = self.model(Xbatch)  # 모델 예측
                prob = F.softmax(output, dim=-1)  # softmax를 통해 확률로 변환
                yhat = np.append(yhat, prob.cpu().numpy())  # 확률을 yhat에 저장
        return yhat.reshape(-1, self.nclasses)  # nclasses로 크기 재조정
