...
from pytorch_pretrained_bert.modeling import BertForQuestionAnsweringQADA
...
def comb_adversarial_training_stage(args, target_train_features, target_train_examples, source_data_iter,
            eval_features, eval_examples, removed_feature_index, new_generated_train_features, model, 
            batcher, epoch, device, best_acc, best_f1, logger):
    ...
    train_dataloader = DataLoader(train_data, sampler=BERTRandomSampler(train_data), batch_size=args.train_batch_size, \
                                  collate_fn=batcher.batching_fn, num_workers=args.num_workers, worker_init_fn=worker_init_fn)
    ...
    for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
        ...
        input_masks = torch.cat((input_masks, input_masks_source), 0).to(device)
        segment_ids = torch.cat((segment_ids, segment_ids_source), 0).to(device)
        start_positions = torch.cat((start_positions, start_positions_source), 0).to(device)
        end_positions = torch.cat((end_positions, end_positions_source), 0).to(device)
        synonym_ids = torch.cat((synonym_ids, synonym_ids_source), 0).to(device)
        synonym_coeffs = torch.cat((synonym_coeffs, synonym_coeffs_source), 0).to(device)

        loss = model.forward_ours(input_ids, segment_ids, input_masks, start_positions,
                end_positions, synonym_ids, synonym_coeffs, args.cutoff_ratio, args.lambda_c)
...
def prepare_model(args, device):
    ...
    model = BertForQuestionAnsweringQADA.from_pretrained(args.bert_model, state_dict=model_state_dict, args=args)
    model.to(device)
    return model
...
def adaptation_stage(args, tokenizer, device, logger, debug=False):
    ...
    logger.info("***** Building Dirichlet Synonym Dictionary *****")
    lengths_1_order, lengths_2_order, joint_synonym_matrix = build_joint_synonym_matrix(args, tokenizer)
    dirichlet_cache = build_dirichlet_coeff_cache(args, lengths_1_order, lengths_2_order, joint_synonym_matrix)
    batcher = DirichletBatcher(args, lengths_1_order, lengths_2_order, joint_synonym_matrix, 
                                args.dirichlet_ratio, dirichlet_cache, True)
    ...
    source_data = TensorDataset(source_input_ids, source_input_mask, source_segment_ids, source_start_positions, source_end_positions)
    source_dataloader = DataLoader(source_data, sampler=InfiniteSampler(source_data), batch_size=args.train_batch_size, \
                                   collate_fn=batcher.batching_fn, num_workers=args.num_workers, worker_init_fn=worker_init_fn)
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        best_acc, best_f1, final_acc, final_f1 = comb_adversarial_training_stage(args, train_features, train_examples,
                source_data_iter, eval_features, eval_examples, removed_feature_index, new_generated_train_features,
                model, batcher, epoch, device, best_acc, best_f1, logger)
        logger.info('\n')
...
def main(debug=False):
    ...
    parser.add_argument('--cache_size', type=int, default=100, help="QADA: Cache size for Dirichlet sampling")
    parser.add_argument('--dirichlet_ratio', type=float, default=0.0, help="QADA: Dirichlet sampling ratio for training")
    parser.add_argument('--cutoff_ratio', type=float, default=0.0, help="QADA: Context cutoff ratio for training")
    parser.add_argument('--lambda_c', default=0.0, type=float, help='QADA: scaling facotor for contrastive loss')
