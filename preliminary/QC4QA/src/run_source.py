...
def prediction_stage(args, device, tokenizer, logger, debug=False):
    """
    저장된 체크포인트 모델을 로드하고 
    predict 파일을 읽어 evaluation_stage를 호출하여 예측 성능(EM/F1)을 로그로 출력함.
    """
    ...
    # prediction 파일에서 feature와 example 읽기
...
def evaluate_acc_and_f1(predictions, raw_data, logger, threshold=-1, all_probs=None):
    """
    predictions(qa id → 답)과 원본 데이터(raw_data)를 비교해 Exact Match(EM)와 F1을 계산
    """
    ...
    for sample in raw_data:
        ...
        if not eval_threshold or (eval_threshold and all_probs[sample.qas_id] >= threshold):
            ...
            # 여러개의 ground_truths 중에서, prediction과 가장 잘 맞는(최고 점수) 정답에 대한 점수를 반환
...
def keep_high_prob_samples(all_probs, all_features, prob_threshold, removed_feature_index, all_indices,keep_generated=False):
    """
    모델 예측 확률(all_probs)을 기준으로 threshold 이상인 feature들만 골라 새 학습용 feature 리스트를 만듦
    """
    new_train_features = []
    for i, feature in enumerate(all_features):
        if keep_generated:
            # removed_feature_index에 포함되지 않은(즉 아직 생성되지 않은) 원문(example_index)에 대해서만, 
            # 확률이 thresh 이상이면 샘플을 선택
            # 이전에 생성한 샘플을 유지하면서 중복으로 여러 샘플 생성되는걸 방지
            ...
            # 생성된 모든 원문에 대해서 thresh이상이면 샘플을 선택(중복 허용)
...
def compare_performance(args, best_acc, best_f1, acc, f1, model, logger):
    """
    현재 EM/F1이 이전 최고 EM/F1보다 높으면 모델을 저장하고 최고 EM/F1 갱신
    """
    if not (best_f1 is None or best_acc is None):
        if best_acc < acc:
            # 현재 성능이 이전 최고 성능보다 높으면 모델 저장하고 성능 갱신
            ...
            # 현재 성능이 이전 최고 성능보다 낮으면 모델 저장하지 않음
...
def evaluation_stage(args, eval_examples, eval_features, device, model, logger, generate_prob_th=0.6,
        removed_feature_index=None, global_step=None, best_acc=None, best_f1=None, generate_label=None):
    """
    주어진 모델로 eval_features에 대해 예측을 수행하여 raw prediction을 수집하고, 
    write_predictions를 호출해 최종 예측/nbest를 생성.
    generate_label=True->새로운 학습 샘플을 생성 / 아니면 evaluate_acc_and_f1로 성능 산출
    """
    ...
    if global_step:
        # Global step이 주어지면, 예측/nbest 파일명을 global step 포함해 생성
        ...
        # Global step이 주어지지 않으면, 기본 파일명 사용
        output_prediction_file = os.path.join(args.output_dir, 'predictions.json')
        output_nbest_file = os.path.join(args.output_dir, 'nbest_predictions.json')
    # write_predictions를 호출해 최종 예측/nbest 생성
    ...
    if generate_label:
        # generate_label이 True이면, 새로운 학습 샘플을 생성(keep_high_prob_samples 호출)
        # 확률이 generate_prob_th 이상인 샘플을 선택해 start/end position을 부여
        ...
        # generate_label이 False이면, EM/F1 계산
...
def generate_self_training_samples(args, train_examples, train_features, device, model, removed_feature_index,
    ...
    logger.info('***** Generating training data for this epoch / 학습 데이터 생성 *****')
    """
    self-training(자기학습)용으로 현재 epoch에서 생성된 높은 확률의 타깃 샘플을 얻음
    현재 모델로부터 확률이 높은 예측을 뽑아 self‑training용 학습 샘플로 만드는 역할
    """

    if args.keep_previous_generated:
        # 이전 epoch에서 생성된 샘플을 유지하면서, 이번 epoch에서 새로 생성된 샘플을 추가
        # removed_feature_index에 포함되지 않은 원문에 대해서만 확률이 thresh 이상이면 샘플을 선택
        ...
        # 이번 epoch에서 새로 생성된 샘플 중, 확률이 thresh 이상이면 샘플을 선택
        train_features_removed_previous = train_features
    
    # 현재 모델로부터 확률이 높은 예측을 뽑아 self‑training용 학습 샘플로 만듦
...
def prepare_model(args, device):
    """
    지정한 사전학습 BERT 모델을 로드하여 device로 이동시킴
    """
...
def training_stage(args, tokenizer, device, logger, debug=False):
    """
    전체 학습 루프를 담당: 모델 준비, 옵티마이저 설정, 학습/검증 데이터 읽기, 
    DataLoader 구성, epoch별 학습 및 주기적 평가 수행, 최종 모델 저장.
    """
    ...
    for _ in trange(int(args.num_train_epochs), desc="Epoch"):
        for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
            ...
            # 역전파->기울기 계산, optimizer.step() 호출 전까지 기울기 누적
            loss.backward()
...
def main(debug=False):
    ...
    parser.add_argument('--logger_path', type=str, default='../../logs/bert', help='The path to save log of current program')
